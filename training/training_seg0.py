...
from network import SpaceTempUNetSeg0

def train_unet_seg0(kinetic_resume_path=None, resume_path=None, device=torch.device("cuda:0")):
    """ 
    ...
    wandb.init(project="DynamicPet_segmentation", config=os.path.join(root_path, "config/config_seg0.yaml"))
    wandb_logger = WandbLogger(project="DynamicPet_segmentation", config=os.path.join(root_path, "config/config_seg0.yaml"))

    # Set up the UNet model
    wandb.config["device"] = device

    # Set up the UNet model
    unet = SpaceTempUNetSeg0(wandb.config)
...
def test_unet_seg0(checkpoint_path, device=torch.device("cuda:0")):
    """
    ...
    wandb_logger = WandbLogger(project="DynamicPet_segmentation", config=os.path.join(root_path, "config/config_seg0.yaml"))
    unet = SpaceTempUNetSeg0(wandb.config, device=device)
