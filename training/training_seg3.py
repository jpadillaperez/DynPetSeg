...
from network import SpaceTempUNetSeg3

def train_unet_seg3(kinetic_resume_path=None, resume_path=None, device=torch.device("cuda:0")):
    """ 
    ...
    wandb.init(project="DynamicPet_segmentation", config=os.path.join(root_path, "config/config_seg3.yaml"))
    wandb_logger = WandbLogger(project="DynamicPet_segmentation", config=os.path.join(root_path, "config/config_seg3.yaml"))

    # Set up the UNet model
    wandb.config["device"] = device

    # Set up the UNet model
    unet = SpaceTempUNetSeg3(wandb.config)

    #TODO: Change depending on structure of SpaceTempUNetSeg3
...
def test_unet_seg3(checkpoint_path, device=torch.device("cuda:0")):
    """
    ...
    wandb_logger = WandbLogger(project="DynamicPet_segmentation", config=os.path.join(root_path, "config/config_seg3.yaml"))
    unet = SpaceTempUNetSeg3(wandb.config, device=device)
