...
class Bas2TrainingModule(BaseTrainingModule):
  def __init__(self, config):
    super(Bas2TrainingModule, self).__init__(config)

    #------------ Segmentation Network ------------
    if self.config["segmentation_network"] == "UNet":
      self.model = UNet_monai(spatial_dims = 2, in_channels=1, out_channels=self.config["output_size_seg"], 
      ...
      self.model = SegResNetDS_monai(spatial_dims=2, init_filters=32, 
                                      in_channels=1, out_channels=self.config["output_size_seg"], 
                                      act='relu', norm='batch', 
  ...
  def forward_implementation(self, input):
    seg_logits = self.model(input["batch"][4])
  ...
  def load_from_checkpoint(self, kinetic_ckpt=None, full_ckpt=None):
    if kinetic_ckpt is not None:
      print("Error: Kinetic checkpoint was provided but full checkpoint should be given for Baseline 2 experiment.")
      exit(1)
