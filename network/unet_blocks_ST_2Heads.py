...
class EncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, bottleneck, config):
        ...
        if max_pool_time:       
            ...
            self.pooling = nn.MaxPool3d(kernel_size=(1, 2, 2), stride=2, padding=0)
...
class UNet_ST_2Heads(nn.Module):
    def __init__(self, in_channels, out_channels_kin, out_channels_seg, config):
        super(UNet_ST_2Heads, self).__init__()

        print("Instantiating Spatio-Temporal UNet with two decoders")
        
        self.encoder = Encoder(in_channels, config)
        self.decoder_kin = Decoder(in_channels, out_channels_kin, config)
        self.decoder_seg = Decoder(in_channels, out_channels_seg, config)

    def forward(self, x):
        y, h = self.encoder(x)
        z1 = self.decoder_kin(y, h)
        z2 = self.decoder_seg(y, h)
        return z1, z2
